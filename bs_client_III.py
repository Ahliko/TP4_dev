...
try:
    msg = input("Saisie ton calcul : ")
    if type(msg) is not str:
        raise TypeError
    regex = re.findall(r'/(-?([1-9][0-9]{0,5}|0))((\+|\-|\*)(-?([1-9][0-9]{0,5}|0)))+/g', msg)
    if regex is None:
        logging.error("Erreur ! Le message doit contenir soit 'meo' soit 'waf' !")
        exit(1)
    elif len(regex) > 1:
        logging.error("Erreur ! Le message ne doit contenir qu'un seul 'meo' ou 'waf' !")
        exit(1)
    else:
        numbers = re.findall(r"-?[0-9]+", msg)
        if not all(-100000 <= int(num) <= 100000 for num in numbers):
            logging.error("Erreur ! Les nombres doivent être compris entre -100000 et 100000 !")
            exit(1)
except TypeError:
